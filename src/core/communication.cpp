...
void mpi_send_vs_relative(int pnode, int part, int vs_relative_to,
                          double vs_distance, double *rel_ori, double* vs_theta) {
  ...
  if (pnode == this_node) {
    ...
    p->p.vs_virtual_site_theta = vs_theta;
    for (int i = 0; i < 4; i++) {
    ...
    MPI_Send(vs_theta, 1, MPI_DOUBLE, pnode, SOME_TAG, comm_cart);
  }
...
void mpi_send_vs_relative_slave(int pnode, int part) {
  ...
  if (pnode == this_node) {
    ...
    MPI_Recv(p->p.vs_relative_rel_orientation, 4, MPI_DOUBLE, 0, SOME_TAG,
             comm_cart, MPI_STATUS_IGNORE);
    MPI_Recv(p->p.vs_virtual_site_theta, 1, MPI_DOUBLE, 0, SOME_TAG,
             comm_cart, MPI_STATUS_IGNORE);
  }
